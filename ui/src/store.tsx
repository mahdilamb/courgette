  ...
  switch (action.type) {
    ...
    case "SET_SCENARIO_TAGS":
      return {
        ...
        builder: {
          ...state.builder,
          scenarios: state.builder.scenarios.map((s) =>
            ...
            s.id === action.scenarioId
              ? {
                  ...
                  steps: s.steps.map((st) =>
                    st.id === action.stepId ? { ...st, [action.field]: action.value } : st
                  ),
                }
              : s
          ),
          ...
          scenarios: state.builder.scenarios.map((s) =>
            s.id === action.scenarioId
              ? {
                  ...s,
                  steps: s.steps.map((st) =>
                    st.id === action.stepId ? { ...st, data_table: action.table } : st
                  ),
                }
              : s
          ),
    ...
    case "LOAD_FEATURE": {
      ...
      const scenarios = (feat.scenarios || []).map((sc) => ({
        id: newId(),
        name: sc.name,
        type: (sc.type || "Scenario") as "Scenario" | "Scenario Outline",
        tags: [],
        steps: sc.steps.map((s) => ({ id: newId(), keyword: s.keyword, text: s.text, data_table: s.data_table })),
