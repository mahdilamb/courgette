...
class ContextKeyError(StepError):
    ...
    def __init__(
        ...
        # Suggest steps that might set the missing key
        suggestions = _suggest_steps_for_key(key)
        if suggestions:
            details.append("")
            details.append("  Try adding one of these steps before this one:")
            for s in suggestions:
                details.append(f"    Given {s}")
...
def _suggest_steps_for_key(key: str) -> list[str]:
    """Find registered Given steps whose source code sets the given context key.

    Scans the function source for ``context["key"]``, ``context.key =``,
    or ``context.setdefault("key"`` patterns.
    """
    import inspect
    ...
    for defn in registry.steps:
        # Only suggest Given (context-setting) steps
        if defn["keyword_type"] not in ("context", None):
            continue
        func = defn["func"]
        try:
        ...
        # Check if the function sets this key in context
        patterns = [
            f'context["{key}"]',
            f"context['{key}']",
            f"context.{key}",
            f'context.setdefault("{key}"',
            f"context.setdefault('{key}'",
        ]
        if any(pat in source for pat in patterns):
            # Check it's an assignment, not just a read
            assign_patterns = [
                f'context["{key}"] =',
                f"context['{key}'] =",
                f"context.{key} =",
            ...
            if any(pat in source for pat in assign_patterns):
                pat = defn["pattern"]
                display = pat.pattern if hasattr(pat, "pattern") else str(pat)
