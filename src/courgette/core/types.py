...
class Context(dict[str, Any]):
    """Scenario execution context — a dict that also supports attribute access.

    This is a plain ``dict`` subclass, so ``context["key"]``,
    ``context.get("key")``, and ``context.setdefault(...)`` all work.
    It also supports ``context.key`` for behave compatibility.
    """
